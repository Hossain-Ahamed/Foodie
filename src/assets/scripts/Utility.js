...
const isValidAddress = (divisionId, districtId, upazillaId, postID, PostCode) => {
    ...
    const PostOffice = PostCodes.postcodes.find(value => value.id.toString() === postID);
    if (
        
        Division && District && Upazilla && PostOffice && PostCode &&
...
export const getPostcode = (id) => {
    const selectedPostcode = PostCodes.postcodes.find(postcode => postcode.id.toString() === id.toString());
